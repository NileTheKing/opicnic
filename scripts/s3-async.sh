...
# 사용: scripts/s3-async.sh [동시 사용자=30] [지속 초=300] [라벨=after]  → docs/performance/<오늘>/s3-async-<라벨>.txt
set -euo pipefail
...
sampler() { while true; do curl -s "$BASE/actuator/prometheus" | awk -v t="$(date +%T)" '/^opicnic_worker_(queued|in_flight) /{printf "%s %s %s\n", t, $1, $2}'; sleep 5; done; }
...
sub=[float(r[4]) for r in rows]; comp=[float(r[5]) for r in rows if r[3]=='202']; codes=[r[3] for r in rows]
...
print(f"# 접수 HTTP: 202={codes.count('202')} 비202={len(codes)-codes.count('202')}  (SLO 100% 202)")
