...
- [x] **버그: 실제 LLM 429·5xx를 못 알아본다** (2026-09-23 발견·수정) — 메시지 "HTTP 429 - {...}"를 "429"로 시작하는지만 봐서 놓침. 수정: `on-http-codes: [429]`, 메시지 앞 상태 코드를 두 형식 다 파싱(`ExternalCallMetrics.statusOf`), 운영과 같은 Spring AI 오류 처리기를 직접 돌리는 테스트(`SpringAiErrorFormatTest`). LLM 400을 영구 실패로 넣는 건 보류 — Groq JSON 모드는 출력 형식 실패도 400(json_validate_failed)으로 줘서 다시 하면 풀릴 수 있다
