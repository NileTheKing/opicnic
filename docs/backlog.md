...
- [ ] **접수 경로를 워커의 DB 사용에서 보호** — 슬롯 스윕에서 100명 동시 시 워커 트랜잭션이 커넥션 풀(10)을 다 물어 접수(잡 행 INSERT)가 8초 대기. 후보: 풀 확대(슬롯과 비례, 30~50) + 워커가 동시에 쓸 커넥션 상한(세마포어) / 접수용 DataSource 분리 / 워커 트랜잭션 짧게(문항 조회를 캐시로). 고친 뒤 100명 조합 재측정. 이게 "워커가 바쁠수록 접수 약속이 늦어진다"는 구조 문제라 슬롯 값보다 먼저
