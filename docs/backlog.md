...
- [ ] 워커 동시 상한(`opicnic.worker.concurrency`, 지금 30) 산정 실험 — S3에서 30명 연속 제출 시 처리 중이 상한에 붙고 큐 max 40, 완료 p95가 단독 대비 +8s. 상한을 올리면서 우리 쪽(힙·CPU·HikariCP pending·R2 읽기)이 어디서 먼저 무너지나로 정한다 — 유료 제공자 가정이라 외부 한도는 기준 아님.  8/31 500VU OOM 87회의 근본 원인이던 "제출 경로 동시성 무제한"은 동기 경로 제거로 소멸했고, 이제 상한은 워커 세마포어 하나. 값은 부하테스트로
