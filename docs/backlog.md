...
- [ ] **재시도 범위가 너무 넓다.** LLM만 실패해도 재시도 루프가 STT부터 다시 돈다. 8/31 측정에서 15문항 × 재시도 2회로 STT 호출이 43건이 되어 Whisper RPM 20을 스스로 초과했고 429가 21번 났다. STT 결과를 재사용하도록 재시도 단위를 쪼갤 것
- [ ] **동기/비동기 결정 대기.** TPM 8K에서 콤보 3문항이 약 17.5K 토큰이라 rate limit 창을 분 단위로 기다려야 하는데, 동기 요청으로는 감당이 안 된다. 다만 이 근거는 제공자에 딸려 있다 — 토큰을 더 줄이거나 TPM이 큰 제공자로 옮기면 처리 시간이 5~10초로 돌아와 동기도 성립한다. **제공자 결정이 먼저, 비동기 결정이 그 다음**
